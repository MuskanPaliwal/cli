...
A cloud dispatch covers repos placed in exactly one jurisdiction and is
generated and stored by that jurisdiction's cell. Without --jurisdiction it
uses your home jurisdiction (see 'entire auth status'); a repo mirrored only
elsewhere is reported as not found there, so pass --jurisdiction <slug> to
target the jurisdiction the repo is placed in.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if shouldRunDispatchWizardForCommand(cmd.Flags().NFlag(), isTerminalStdin(os.Stdin), interactive.IsTerminalWriter(cmd.OutOrStdout())) {
				...
				opts, err = parseDispatchFlags(cmd, flagLocal, flagSince, flagUntil, flagAllBranches, flagRepos, flagVoice, flagJurisdiction, flagInsecureHTTPAuth)
			}
			if err != nil {
			...
			if err := runDispatchCommand(cmd.Context(), cmd.OutOrStdout(), opts); err != nil {
				...
				return describeDispatchRepoNotFound(cmd.Context(), err)
			}
			return nil
		},
	}
	...
	cmd.Flags().StringVarP(&flagJurisdiction, "jurisdiction", "j", "", "jurisdiction slug (e.g. us, eu) whose cell generates and stores the cloud dispatch; defaults to your home jurisdiction")
